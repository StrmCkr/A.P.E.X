package io.github.strmckr.apex.tuples;
...
public class Tuples {
    ...
    public static int directTupleRadixCap() {
        int tupleBits = Math.max(ApexEngine.DIRECT_TUPLE_BITS, ApexEngine.DIRECT_TUPLE_CONTIGUOUS_BITS);
    ...
    public static boolean tupleSpaceFitsDirectPass(long entropyMask) {
        ...
        return tupleBits > 1 && tupleBits <= ApexEngine.DIRECT_TUPLE_BITS;
    }

    public static boolean tupleSpaceFitsDirectPass(long entropyMask, int size) {
        int tupleBits = Long.bitCount(entropyMask);
        int maxTupleBits = Math.max(ApexEngine.DIRECT_TUPLE_BITS, ApexEngine.DIRECT_TUPLE_CONTIGUOUS_BITS);
        ...
        if (tupleBits <= ApexEngine.DIRECT_TUPLE_BITS) {
            return true;
        }

        return tupleBits <= ApexEngine.DIRECT_TUPLE_CONTIGUOUS_BITS &&
    ...
    public static boolean directTupleUsesInPlace(int size, boolean preferInPlace) {
        return preferInPlace || size <= ApexEngine.DIRECT_TUPLE_IN_PLACE_MAX_RECORDS;
    }
    ...
    public static int tupleIndex(long key, long entropyMask) {
        ...
        return (int) Long.compress(key ^ ApexEngine.KEY_ORDER_XOR, entropyMask);
    }
        ...
        if (tupleShift >= 0) {
            for (int bin = 0; bin < radix; bin++) {
                ...
                while (i < end) {
                    ...
                    long key = dst.get(ApexEngine.LONG, recordOffset);
                    ...
                    long value = dst.get(ApexEngine.LONG, recordOffset + 8);
                    long targetKey = dst.get(ApexEngine.LONG, targetOffset);
                    long targetValue = dst.get(ApexEngine.LONG, targetOffset + 8);

                    dst.set(ApexEngine.LONG, targetOffset, key);
                    dst.set(ApexEngine.LONG, targetOffset + 8, value);
                    dst.set(ApexEngine.LONG, recordOffset, targetKey);
                    dst.set(ApexEngine.LONG, recordOffset + 8, targetValue);
                }
            }
        } else {
            for (int bin = 0; bin < radix; bin++) {
    ...
    public static void tupleCountingPassSegments(
        ...
        if (shift >= 0) {
            while (p <= unrolledEnd) {
                long k0 = source.get(ApexEngine.LONG, p);
                long k1 = source.get(ApexEngine.LONG, p + 16);
                long k2 = source.get(ApexEngine.LONG, p + 32);
                long k3 = source.get(ApexEngine.LONG, p + 48);
            ...
            while (p < end) {
                long k = source.get(ApexEngine.LONG, p);
        ...
        if (shift >= 0) {
            while (p <= unrolledEnd) {
                long k0 = source.get(ApexEngine.LONG, p);
                long v0 = source.get(ApexEngine.LONG, p + 8);
                long k1 = source.get(ApexEngine.LONG, p + 16);
                long v1 = source.get(ApexEngine.LONG, p + 24);
                long k2 = source.get(ApexEngine.LONG, p + 32);
                long v2 = source.get(ApexEngine.LONG, p + 40);
                long k3 = source.get(ApexEngine.LONG, p + 48);
                long v3 = source.get(ApexEngine.LONG, p + 56);

                int bin0 = (int) (((k0 ^ keyOrderXor) >>> shift) & mask);
                long targetPos0 = targetBase + ((long) counts[bin0]++ << 4);
                target.set(ApexEngine.LONG, targetPos0, k0);
                target.set(ApexEngine.LONG, targetPos0 + 8, v0);

                int bin1 = (int) (((k1 ^ keyOrderXor) >>> shift) & mask);
                long targetPos1 = targetBase + ((long) counts[bin1]++ << 4);
                target.set(ApexEngine.LONG, targetPos1, k1);
                target.set(ApexEngine.LONG, targetPos1 + 8, v1);

                int bin2 = (int) (((k2 ^ keyOrderXor) >>> shift) & mask);
                long targetPos2 = targetBase + ((long) counts[bin2]++ << 4);
                target.set(ApexEngine.LONG, targetPos2, k2);
                target.set(ApexEngine.LONG, targetPos2 + 8, v2);

                int bin3 = (int) (((k3 ^ keyOrderXor) >>> shift) & mask);
                long targetPos3 = targetBase + ((long) counts[bin3]++ << 4);
                target.set(ApexEngine.LONG, targetPos3, k3);
                target.set(ApexEngine.LONG, targetPos3 + 8, v3);
            ...
            while (p < end) {
                long k = source.get(ApexEngine.LONG, p);
                long v = source.get(ApexEngine.LONG, p + 8);

                int bin = (int) (((k ^ keyOrderXor) >>> shift) & mask);
                long targetPos = targetBase + ((long) counts[bin]++ << 4);

                target.set(ApexEngine.LONG, targetPos, k);
                target.set(ApexEngine.LONG, targetPos + 8, v);
                p += ApexEngine.RECORD_BYTES;
            }
        } else {
            while (p <= unrolledEnd) {
    ...
    public static void parallelTupleCountingPassSegments(
        ...
        int radixThisPass = mask + 1;
        int[][] threadOffsets = new int[ApexEngine.THREADS][radixThisPass];
        ArrayList<Future<?>> futures = new ArrayList<>(ApexEngine.THREADS);
        int chunk = size / ApexEngine.THREADS;
        ...
        for (int t = 0; t < ApexEngine.THREADS; t++) {
            ...
            futures.add(ApexEngine.POOL.submit(() -> {
                ...
                int start = tid * chunk;
                int end = (tid == ApexEngine.THREADS - 1) ? size : start + chunk;
                long p = sourceBase + ((long) start << 4);
                long limit = sourceBase + ((long) end << 4);
                long unrolledLimit = limit - (4L * ApexEngine.RECORD_BYTES);
                long keyOrderXor = ApexEngine.KEY_ORDER_XOR;
