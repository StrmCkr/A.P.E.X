...
public final class DataTopology {
    ...
    public static void printTopology(DataMode mode) {
        ...
        Reporters.println("Data mode        : " + mode);
        Reporters.println("Topology class   : " + topologyClass(mode));
        Reporters.println("Description      : " + description(mode));
        Reporters.println("Apex behavior    : " + apexBehavior(mode));
        Reporters.println("Entropy example  : " + entropyExample(mode));

        switch (mode) {

            case RANDOM ->
                Reporters.println("Example          : Uniform entropy across all radix regions");

            case SORTED ->
                Reporters.println("Example          : Strict ascending monotonic ordering");

            case REVERSE ->
                Reporters.println("Example          : Strict descending monotonic ordering");

            case NEARLY_SORTED ->
                Reporters.println("Example          : Mostly sorted with sparse local disorder");

            case DUPLICATES ->
                Reporters.println("Example          : Large repeated key populations");

            case LOW_BITS_ONLY ->
                Reporters.println("Example          : Entropy only appears in low-order bits");

            case HIGH_BITS_ONLY ->
                Reporters.println("Example          : Entropy concentrated in highest-order bits");

            case ZIPFIANISH ->
                Reporters.println("Example          : Heavy skew toward dominant values");

            case ALL_EQUAL ->
                Reporters.println("Example          : Every key is identical");

            case EMPTY ->
                Reporters.println("Example          : No records");

            case SINGLE_ELEMENT ->
                Reporters.println("Example          : Single-record dataset");

            case TWO_ELEMENTS_SORTED ->
                Reporters.println("Example          : Already sorted pair");

            case TWO_ELEMENTS_REVERSED ->
                Reporters.println("Example          : Minimal reversed pair");

            default ->
                Reporters.println("Example          : Specialized entropy topology");
        }
    }
