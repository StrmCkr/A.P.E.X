package io.github.strmckr.apex.tinysorts;
...
public class TinySort {
    ...
    public static void tinyPartitionBitSort(MemorySegment dst, long startPos, int size, Scratch sc) {
        ...
        for (; i < unrolledEnd; i += 4) {
            ...
            k[i]     = dst.get(ApexEngine.LONG, p0);
            v[i]     = dst.get(ApexEngine.LONG, p0 + 8);
            
            k[i + 1] = dst.get(ApexEngine.LONG, p1);
            v[i + 1] = dst.get(ApexEngine.LONG, p1 + 8);
            
            k[i + 2] = dst.get(ApexEngine.LONG, p2);
            v[i + 2] = dst.get(ApexEngine.LONG, p2 + 8);
            
            k[i + 3] = dst.get(ApexEngine.LONG, p3);
            v[i + 3] = dst.get(ApexEngine.LONG, p3 + 8);
        }
        
        for (; i < size; i++) {
            long p = base + ((long) i << 4);
            k[i] = dst.get(ApexEngine.LONG, p);
            v[i] = dst.get(ApexEngine.LONG, p + 8);
        }

        // Select the local sort strategy by partition size.
        if (size < 24) {
             // 1-23 records: insertion sort
              insertionSmall(k, v, 0, size);
        } else if (size < 64) {
          // 24-63 records: binary insertion sort
        	binaryInsertionSmall(k, v, 0, size);
        } else if (size < 128) {
        	// 64-127 records: median-of-three quicksort
            quickSort(k, v, 0, size - 1);
        } else if (size < 192) {
            // 128-191 records: three-way quicksort for duplicate-heavy partitions
            threeWayQuickSort(k, v, 0, size - 1);
            }        
        else {
			// Larger tiny-route partitions: local MSD radix sort
        	MsdRadix8KV.sort(k, v, sc.k2, sc.v2, 0, size, 56);
		}
        

        // Stride-Pipelined Writeback Staged in 4-Way Blocks
        // Non-temporal writeback pass
        ...
        for (; i < vectorEnd; i += 8) {
            dst.set(ApexEngine.LONG, p,       k[i]);     dst.set(ApexEngine.LONG, p + 8,   v[i]);
            dst.set(ApexEngine.LONG, p + 16,  k[i + 1]); dst.set(ApexEngine.LONG, p + 24,  v[i + 1]);
            dst.set(ApexEngine.LONG, p + 32,  k[i + 2]); dst.set(ApexEngine.LONG, p + 40,  v[i + 2]);
            dst.set(ApexEngine.LONG, p + 48,  k[i + 3]); dst.set(ApexEngine.LONG, p + 56,  v[i + 3]);
            dst.set(ApexEngine.LONG, p + 64,  k[i + 4]); dst.set(ApexEngine.LONG, p + 72,  v[i + 4]);
            dst.set(ApexEngine.LONG, p + 80,  k[i + 5]); dst.set(ApexEngine.LONG, p + 88,  v[i + 5]);
            dst.set(ApexEngine.LONG, p + 96,  k[i + 6]); dst.set(ApexEngine.LONG, p + 104, v[i + 6]);
            dst.set(ApexEngine.LONG, p + 112, k[i + 7]); dst.set(ApexEngine.LONG, p + 120, v[i + 7]);
        ...
        for (; i < size; i++) {
            dst.set(ApexEngine.LONG, p, k[i]);
            dst.set(ApexEngine.LONG, p + 8, v[i]);
    ...
    /**
     * Three-way unsigned quicksort for duplicate-heavy small partitions.
     */
