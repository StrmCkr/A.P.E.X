package io.github.strmckr.apex.lsd;
...
public class LsdBucketPlan {
	  ...
	  public static int buildLsdCyclePlan(
	        ...
	        if (packedCycles < contiguousCycles || ApexEngine.PACKED_TUPLE_CYCLES) {
	            long packedScore = scorePackedTupleCyclePlan(variableMask, size, cfg.lsdBits);
	            if (ApexEngine.PACKED_TUPLE_CYCLES || packedScore < bestScore) {
	        ...
	        if (maxStaggerBits > cfg.lsdBits && size >= ApexEngine.STAGGER_TUPLE_MIN_RECORDS &&
	                !ApexEngine.STAGGER_TUPLE_COST_MODEL) {
	        ...
	        } else if (maxStaggerBits > cfg.lsdBits && size >= ApexEngine.STAGGER_TUPLE_MIN_RECORDS) {
	  ...
	  static int packedTupleCycleCount(long variableMask, SortConfig cfg) {
	  ...
	  static int maxStaggerTupleBits(SortConfig cfg) {
	        if (!ApexEngine.STAGGER_TUPLE_CYCLES) {
	            return 0;
	        }

	        int configured = Math.min(ApexEngine.STAGGER_TUPLE_BITS, ApexEngine.DIRECT_TUPLE_BITS);
	        configured = Math.min(configured, ApexEngine.MAX_DIRECT_TUPLE_BITS);
	  ...
	  static long scorePackedTupleCyclePlan(long variableMask, int size, int bitsPerCycle) {
	        int cycleBits = Math.max(1, Math.min(ApexEngine.MAX_DIRECT_TUPLE_BITS, bitsPerCycle));
	  ...
	  public static void sortMsdBucketsWithLsdRadix(
	                    ...
	                    try {
	                        ...
	                        for (;;) {
	                            ...
	                            int adaptiveBatch = (remainingWorkCount <= 64) ? 1 : 
	                                                Math.max(4, Math.min(2048, (int)(remainingWorkCount / ApexEngine.THREADS)));
	        ...
	        if (ApexEngine.LSD_WORK_STEALING) {
	            int[] workBuckets = MsdBucketPlanner.buildLsdWorkBucketsByDescendingSize(plan, cfg);
	            ...
	            for (int t = 0; t < ApexEngine.THREADS; t++) {
	                ...
	                futures.add(ApexEngine.POOL.submit(() -> {
	                    try {
	                        ...
	                        for (int b = tid; b < cfg.msdBucketCount; b += ApexEngine.THREADS) {
	    ...
	    static void sortOneMsdBucketWithLsdRadix(
	        ...
	        if (size > ApexEngine.MAX_HEAP_SCRATCH_RECORDS &&
	    ...
	    private static void loadHeapPartition(
	        ...
	        if (useHeapUnroll8(size)) {
	            ...
	            for (; i < vectorEnd; i += 8) {
	                keys[i] = dst.get(ApexEngine.LONG, p);
	                values[i] = dst.get(ApexEngine.LONG, p + 8);
	                keys[i + 1] = dst.get(ApexEngine.LONG, p + 16);
	                values[i + 1] = dst.get(ApexEngine.LONG, p + 24);
	                keys[i + 2] = dst.get(ApexEngine.LONG, p + 32);
	                values[i + 2] = dst.get(ApexEngine.LONG, p + 40);
	                keys[i + 3] = dst.get(ApexEngine.LONG, p + 48);
	                values[i + 3] = dst.get(ApexEngine.LONG, p + 56);
	                keys[i + 4] = dst.get(ApexEngine.LONG, p + 64);
	                values[i + 4] = dst.get(ApexEngine.LONG, p + 72);
	                keys[i + 5] = dst.get(ApexEngine.LONG, p + 80);
	                values[i + 5] = dst.get(ApexEngine.LONG, p + 88);
	                keys[i + 6] = dst.get(ApexEngine.LONG, p + 96);
	                values[i + 6] = dst.get(ApexEngine.LONG, p + 104);
	                keys[i + 7] = dst.get(ApexEngine.LONG, p + 112);
	                values[i + 7] = dst.get(ApexEngine.LONG, p + 120);
	    ...
	    private static void storeHeapPartition(
	        ...
	        if (useHeapUnroll8(size)) {
	            int vectorEnd = size - (size & 7);
	            for (; i < vectorEnd; i += 8) {
	                dst.set(ApexEngine.LONG, p, keys[i]);
	                dst.set(ApexEngine.LONG, p + 8, values[i]);
	                dst.set(ApexEngine.LONG, p + 16, keys[i + 1]);
	                dst.set(ApexEngine.LONG, p + 24, values[i + 1]);
	                dst.set(ApexEngine.LONG, p + 32, keys[i + 2]);
	                dst.set(ApexEngine.LONG, p + 40, values[i + 2]);
	                dst.set(ApexEngine.LONG, p + 48, keys[i + 3]);
	                dst.set(ApexEngine.LONG, p + 56, values[i + 3]);
	                dst.set(ApexEngine.LONG, p + 64, keys[i + 4]);
	                dst.set(ApexEngine.LONG, p + 72, values[i + 4]);
	                dst.set(ApexEngine.LONG, p + 80, keys[i + 5]);
	                dst.set(ApexEngine.LONG, p + 88, values[i + 5]);
	                dst.set(ApexEngine.LONG, p + 96, keys[i + 6]);
	                dst.set(ApexEngine.LONG, p + 104, values[i + 6]);
	                dst.set(ApexEngine.LONG, p + 112, keys[i + 7]);
	                dst.set(ApexEngine.LONG, p + 120, values[i + 7]);
	                p += 8L * ApexEngine.RECORD_BYTES;
	            }
	        }
	    ...
	    private static boolean useHeapUnroll8(int size) {
	        if (ApexEngine.LSD_HEAP_UNROLL >= 8) {
	            return true;
	        }

	        return ApexEngine.LSD_HEAP_UNROLL == 0 && size >= ApexEngine.LSD_HEAP_UNROLL_MIN_RECORDS;
	    }
	    ...
	    static void lsdRadixSortPartitionOffHeap(
	        ...
	        try {
	            ApexEngine.LARGE_PARTITION_PERMITS.acquire();
	        ...
	        try {
	            ...
	            ApexEngine.LARGE_PARTITION_PERMITS.release();
	        }
	    }
	    public static boolean bucketHasLsdWork(MsdBucketPlan plan, SortConfig cfg, int b) {
	        if (plan.sizes[b] <= 1) {
	            return false;
	        }
	        ...
	        return plan.bucketFlags[b] == ApexEngine.BUCKET_MIXED &&
	    ...
	    public static int directTupleWorkItemCount(MsdBucketPlan plan, SortConfig cfg) {
	        int count = 0;
	    ...
	    public static boolean preferManyDirectTuplePartitionsInPlace(MsdBucketPlan plan, SortConfig cfg) {
	        int minPartitions = ApexEngine.DIRECT_TUPLE_MANY_PARTITION_MIN;
	        return minPartitions > 0 && directTupleWorkItemCount(plan, cfg) >= minPartitions;
	    }

	    public static boolean bucketHasScheduledLsdWork(MsdBucketPlan plan, SortConfig cfg, int b) {
	        if (!bucketHasLsdWork(plan, cfg, b)) {
	            return false;
	        }
	    ...
	    public static int localMsdShiftForBucket(MsdBucketPlan plan, SortConfig cfg, int b) {
	        return localMsdShiftForBucket(plan, cfg, b, false);
	    }

	    public static int localMsdShiftForBucket(MsdBucketPlan plan, SortConfig cfg, int b, boolean sparseParentPlan) {
	        return localMsdShiftForBucket(plan, cfg, b, localMsdBits(cfg), sparseParentPlan);
	    }

	    public static int localMsdShiftForBucket(MsdBucketPlan plan, SortConfig cfg, int b, int localBits) {
	    ...
	    public static int localMsdShiftForBucket(
	        ...
	        if (!ApexEngine.LOCAL_MSD_REPARTITION) {
	            return -1;
	        }

	        int size = plan.sizes[b];
	        if (size < ApexEngine.LOCAL_MSD_MIN_RECORDS || size < cfg.tinyPartitionThreshold) {
	            return -1;
	        }

	        int plannedPasses = plan.cycleCounts[b] + (plan.tupleTailMasks[b] != 0L ? 1 : 0);
	        if (plannedPasses < ApexEngine.LOCAL_MSD_MIN_PASSES) {
	            return -1;
	        }

	        boolean offHeapSized = size > ApexEngine.MAX_HEAP_SCRATCH_RECORDS;
	        if (!offHeapSized &&
	                !sparseParentPlan &&
	                ApexEngine.LOCAL_MSD_MIN_SHARE_DIVISOR > 0 &&
	                (long) size * ApexEngine.LOCAL_MSD_MIN_SHARE_DIVISOR < plan.totalRecords) {
	            return -1;
	        }
	        ...
	        return windowBits >= ApexEngine.LOCAL_MSD_MIN_WINDOW_BITS ? shift : -1;
	    }

	    public static int localMsdBits(SortConfig cfg) {
	        return ApexEngine.LOCAL_MSD_BITS > 0 ? ApexEngine.LOCAL_MSD_BITS : cfg.msdBits;
	    }

	    public static int localMsdBitsForCandidateCount(SortConfig cfg, int candidateCount) {
	        int bits = localMsdBits(cfg);
	        if (candidateCount <= 0 || ApexEngine.LOCAL_MSD_MAX_CHILDREN <= 0) {
	            return bits;
	        }

	        int childrenPerBucket = Math.max(1, ApexEngine.LOCAL_MSD_MAX_CHILDREN / candidateCount);
	        int cappedBits = 31 - Integer.numberOfLeadingZeros(childrenPerBucket);
	        return Math.max(1, Math.min(bits, cappedBits));
	    }

	    public static int localMsdBucketCount(SortConfig cfg) {
	    ...
	    static boolean tryDominantPrefixCoreSort(
	        ...
	        if (!ApexEngine.DOMINANT_CORE_FAST_PATH ||
	                size <= ApexEngine.MAX_HEAP_SCRATCH_RECORDS ||
	                scratch == dst) {
	            return false;
	        }

	        int candidateCap = Math.max(2, Math.min(256, ApexEngine.DOMINANT_CORE_CANDIDATES));
	        long[] candidates = new long[candidateCap];
	        int[] votes = new int[candidateCap];
	        boolean[] used = new boolean[candidateCap];
	        int sample = Math.min(size, Math.max(1, ApexEngine.DOMINANT_CORE_SAMPLE_RECORDS));
	        long base = startPos << 4;
	        long p = base;

	        for (int i = 0; i < sample; i++) {
	            long key = dst.get(ApexEngine.LONG, p);
	        ...
	        int minPerKey = Math.max(2, size / Math.max(2, ApexEngine.DOMINANT_KEY_MIN_SHARE_DIVISOR));
	        int heavyCount = 0;
	        ...
	        long minRequired = (long) size * Math.max(1, ApexEngine.DOMINANT_CORE_MIN_SHARE_PERCENT);
	        ...
	        while (p < end) {
	            long key = dst.get(ApexEngine.LONG, p);
	            long value = dst.get(ApexEngine.LONG, p + 8);
	            int index = lookupCandidate(tableKeys, tableIndexes, tableUsed, key);
	            int out = index >= 0 ? offsets[index]++ : nonCoreOffset++;
	            long target = scratchBase + ((long) out << 4);

	            scratch.set(ApexEngine.LONG, target, key);
	            scratch.set(ApexEngine.LONG, target + 8, value);
	            p += ApexEngine.RECORD_BYTES;
	        }
