package io.github.strmckr.apex.msd;
...
import io.github.strmckr.apex.histogram.BuildHistogram;
...
public class MsdBucketPlanner {
	...
	public static class MsdBucketPlan {
		...
		public       final int[][] threadScatterOffsets;
	        ...
	        public MsdBucketPlan(SortConfig cfg, int msdShift, long totalRecords) {
	            starts = new long[cfg.msdBucketCount];
	            sizes = new int[cfg.msdBucketCount];
	            threadScatterOffsets = new int[ApexEngine.THREADS][cfg.msdBucketCount];
	   ...
	   public static MsdBucketPlan buildMsdBucketPlan(
	        ...
	        for (int b = 0; b < cfg.msdBucketCount; b++) {
	            ...
	            if (!seenAny || size == 0) {
	                plan.bucketFlags[b] = ApexEngine.BUCKET_EMPTY;
	            } else if (size == 1 || variableMask == 0L) {
	                plan.bucketFlags[b] = ApexEngine.BUCKET_ALL_EQUAL;
	            } else { if (plan.bucketAscending[b]) {
	                    plan.bucketFlags[b] = ApexEngine.BUCKET_ASCENDING;
	                } else if (plan.bucketDescending[b]) {
	                    plan.bucketFlags[b] = ApexEngine.BUCKET_DESCENDING;
	                } else
	                plan.bucketFlags[b] = ApexEngine.BUCKET_MIXED;
	            }

	            if (plan.bucketFlags[b] == ApexEngine.BUCKET_MIXED) {
	 ...
	 public static MsdBucketPlan buildAdaptiveMsdBucketPlan(MemorySegment src, long n, SortConfig cfg) throws Exception {
	  ...
	  public static int largestBucketSize(MsdBucketPlan plan, SortConfig cfg) {
	        int max = 0;
	  ...
	  static void attachLocalMsdPlans(MemorySegment src, long n, SortConfig cfg, MsdBucketPlan plan) throws Exception {
	        ...
	        int rows = ApexEngine.THREADS * candidateCount;
	        ...
	        for (int candidateIndex = 0; candidateIndex < candidateCount; candidateIndex++) {
	            ...
	            int[][] localThreadOffsets = new int[ApexEngine.THREADS][localBucketCount];
	  ...
	  static boolean sparseParentPlan(MsdBucketPlan plan, SortConfig cfg) {
	        int nonEmpty = 0;
	        for (int b = 0; b < cfg.msdBucketCount; b++) {
	            if (plan.sizes[b] != 0) {
	                nonEmpty++;
	  ...
	  static long collapsedPlanVariableMask(MsdBucketPlan plan, SortConfig cfg) {
	  ...
	  public static boolean prefixAboveWindowIsConstant(MemorySegment src, long n, int firstVariableBit) throws Exception {
	        ...
	        ArrayList<Future<Boolean>> futures = new ArrayList<>(ApexEngine.THREADS);
	        long chunk = n / ApexEngine.THREADS;
	        long prefixMask = ~Tools.lowBitsMask(firstVariableBit);
	        long expected = src.get(ApexEngine.LONG, 0) & prefixMask;
	        ...
	        for (int t = 0; t < ApexEngine.THREADS; t++) {
	            ...
	            futures.add(ApexEngine.POOL.submit(() -> {
	                ...
	                while (p <= unrolledEnd) {
	                    if (((src.get(ApexEngine.LONG, p) & prefixMask) != expected) ||
	                            ((src.get(ApexEngine.LONG, p + 16) & prefixMask) != expected) ||
	                            ((src.get(ApexEngine.LONG, p + 32) & prefixMask) != expected) ||
	                            ((src.get(ApexEngine.LONG, p + 48) & prefixMask) != expected)) {
	                        return false;
	                    }
	                    p += 4L * ApexEngine.RECORD_BYTES;
	                }
	        ...
	        static int[] msdShiftCandidates(SortConfig cfg) {
	        ...
	        public static int[] buildLsdWorkBucketsByDescendingSize(MsdBucketPlan plan, SortConfig cfg) {
