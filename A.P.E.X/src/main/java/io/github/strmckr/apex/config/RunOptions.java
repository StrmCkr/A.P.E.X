...
import io.github.strmckr.apex.generator.RecordCountForMode;
...
public class RunOptions {
    ...
    public static class Options {
        ...
        public long tuneRecords = ApexEngine.TUNE_RECORDS;
        public  long warmupRecords = ApexEngine.WARMUP_RECORDS;
        ...
        public  int contiguousDirectTupleBits = Integer.getInteger(
                ...
                ApexEngine.DIRECT_TUPLE_CONTIGUOUS_BITS
        );
        ...
        public  int localMsdMaxChildren = Integer.getInteger("apex.localMsdMaxChildren", ApexEngine.LOCAL_MSD_MAX_CHILDREN);
        public  boolean dominantCoreFastPath = Boolean.parseBoolean(System.getProperty("apex.dominantCore", Boolean.toString(ApexEngine.DOMINANT_CORE_FAST_PATH)));
        public  int dominantCoreSampleRecords = Integer.getInteger("apex.dominantCoreSampleRecords", ApexEngine.DOMINANT_CORE_SAMPLE_RECORDS);
        public  int dominantCoreCandidates = Integer.getInteger("apex.dominantCoreCandidates", ApexEngine.DOMINANT_CORE_CANDIDATES);
        public  int dominantCoreMinSharePercent = Integer.getInteger("apex.dominantCoreMinShare", ApexEngine.DOMINANT_CORE_MIN_SHARE_PERCENT);
        public  int dominantKeyMinShareDivisor = Integer.getInteger("apex.dominantKeyMinShareDivisor", ApexEngine.DOMINANT_KEY_MIN_SHARE_DIVISOR);
        ...
        public String reportingMode = initialReportingMode();
        public String reportFile = System.getProperty("apex.reportFile", "");
        public SortConfig lockedConfig;
    }

    static String initialReportingMode() {
        if (Boolean.getBoolean("apex.quiet") || Boolean.getBoolean("apex.silent")) {
            return "off";
        }
        return Reporters.normalizeMode(System.getProperty("apex.reporting", "summary"));
    }
    ...
    public static void validateOptions(Options options) {
        ...
        if (options.directTupleBits > ApexEngine.MAX_DIRECT_TUPLE_BITS) {
            throw new IllegalArgumentException("tupleBits must be <= " +  ApexEngine.MAX_DIRECT_TUPLE_BITS);
        }

        if (options.contiguousDirectTupleBits > ApexEngine.MAX_DIRECT_TUPLE_BITS) {
        ...
        if (options.staggerTupleBits <= 0 || options.staggerTupleBits > ApexEngine.MAX_DIRECT_TUPLE_BITS) {
    ...
    public static void applyApexSettings(Options options) {
        ...
        ApexEngine.DIRECT_TUPLE_CONTIGUOUS_BITS = options.contiguousDirectTupleBits;
        ApexEngine.LOCAL_MSD_MAX_CHILDREN = options.localMsdMaxChildren;
        ApexEngine.DOMINANT_CORE_FAST_PATH = options.dominantCoreFastPath;
        ApexEngine.DOMINANT_CORE_SAMPLE_RECORDS = options.dominantCoreSampleRecords;
        ApexEngine.DOMINANT_CORE_CANDIDATES = options.dominantCoreCandidates;
        ApexEngine.DOMINANT_CORE_MIN_SHARE_PERCENT = options.dominantCoreMinSharePercent;
        ApexEngine.DOMINANT_KEY_MIN_SHARE_DIVISOR = options.dominantKeyMinShareDivisor;
    }
    ...
    public static void applyApexSettings(
        ...
        ApexEngine.ORDER_FAST_PATH = orderFastPath;
        ApexEngine.SIGNED_KEYS = signedKeys;
        ApexEngine.KEY_ORDER_XOR = signedKeys ? Long.MIN_VALUE : 0L;
        ApexEngine.LSD_WORK_STEALING = lsdWorkStealing;
        ApexEngine.WORK_STEAL_BATCH = workStealBatch;
        ApexEngine.DESCENDING_SCATTER_FAST_PATH = descendingScatterFastPath;
        ApexEngine.PACKED_TUPLE_CYCLES = packedTupleCycles;
        ApexEngine.DIRECT_TUPLE_BITS = directTupleBits;
        ApexEngine.DIRECT_TUPLE_IN_PLACE_MAX_RECORDS = directTupleInPlaceMaxRecords;
        ApexEngine.DIRECT_TUPLE_MANY_PARTITION_MIN = directTupleManyPartitionMin;
        ApexEngine.STAGGER_TUPLE_CYCLES = staggerTupleCycles;
        ApexEngine.STAGGER_TUPLE_COST_MODEL = staggerTupleCostModel;
        ApexEngine.STAGGER_TUPLE_BITS = staggerTupleBits;
        ApexEngine.STAGGER_TUPLE_MIN_RECORDS = staggerTupleMinRecords;
        ApexEngine.LSD_HEAP_UNROLL = lsdHeapUnroll;
        ApexEngine.LSD_HEAP_UNROLL_MIN_RECORDS = lsdHeapUnrollMinRecords;
        ApexEngine.MAX_HEAP_SCRATCH_RECORDS = heapScratchRecords;
        ApexEngine.LOCAL_MSD_BITS = localMsdBits;
        ApexEngine.LOCAL_MSD_MIN_SHARE_DIVISOR = Integer.getInteger(
                "apex.localMsdMinShareDivisor",
                Math.max(2, threads / 2)
        );

        ApexEngine.LARGE_PARTITION_PERMIT_COUNT = largePartitionPermits > 0
                ? largePartitionPermits
                : Math.max(1, threads / 8);
        ApexEngine.LARGE_PARTITION_PERMITS = new Semaphore(ApexEngine.LARGE_PARTITION_PERMIT_COUNT);
    }
    ...
    public static void validateConfig(SortConfig cfg) {
    ...
    public static Options parseOptions(String[] args) {
        ...
        for (String raw : args) {
            ...
            switch (key) {
                ...
                case "standardreporting":
                    options.reportingMode = Reporters.normalizeMode(value);
                    break;
                case "reportfile":
                case "reportpath":
                case "reportoutput":
                ...
                case "quiet":
                case "silent":
                    options.reportingMode = parseBoolean(value) ? "off" : "summary";
                    break;
                case "threads":
        ...
        if (positional.size() >= 5 && options.lockedConfig == null) {
            options.lockedConfig = new SortConfig(
    ...
    public static SortConfig parseConfig(String value) {
